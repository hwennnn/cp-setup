...
def get_polygon_area(xs, ys, take_abs=True, take_double=False):
    # switch to func=triangle_formula if needed for speed
    signed_area = shoelace_formula(xs, ys)
...
largest_prime_factors = get_largest_prime_factors(
    10**6)   # take care that it begins with [1,1,2,...]
primes = [x for i, x in enumerate(
    largest_prime_factors[2:], start=2) if x == i]
...
def modinv(A, n, s=1, t=0, N=0): return (n < 2 and t %
                                         N or modinv(n, A % n, t, s-A//n*t, N or n), -1)[n < 1]
    ...
    def untt(a, n):
        ...
        for i in range(n):
            ...
            for s in range(1 << i):
                ...
                s *= m*2
                for p in range(m):
                    a[s+p], a[s+p+m] = (a[s+p]+a[s+p+m]
                                        ) % MOD, (a[s+p]-a[s+p+m])*w_N % MOD
...
def rand(): return randint(10**128, 10**256)


S = [rand() for _ in range(4)]
def g(x, salt1, salt2): return x * salt1 + salt2
...
def evaluate_rpn(items: Iterable[StackItem]) -> Number:
    ...
    for item in items:
        if isinstance(item, str):
            ...
            arguments = reversed([num_stack.pop()
                                 for _ in range(item.arg_count)])
...
# Credit: https://github.com/tonghuikang/codecomp/blob/master/template/template_math.py
